...
# Connection string from environment variable (configured in Azure Function App Settings)
CONNECTION_STRING = os.getenv('AZURE_STORAGE_CONNECTION_STRING')
if not CONNECTION_STRING:
	raise ValueError('AZURE_STORAGE_CONNECTION_STRING environment variable is required')
CONTAINER_NAME = 'cmsfiles'
...
def read_csv_from_blob(blob_name, **kwargs):
    """Read a CSV file from Azure Blob Storage into a pandas DataFrame."""
    try:
        blob_client = container_client.get_blob_client(blob_name)
        download_stream = blob_client.download_blob()
        return pd.read_csv(BytesIO(download_stream.readall()), **kwargs)
    except Exception as e:
        print(f"Error reading {blob_name}: {e}")
        raise

def write_csv_to_blob(df, blob_name, **kwargs):
    ...
    try:
        ...
        print(f"Error writing {blob_name}: {e}")
        raise

def list_blobs_with_pattern(pattern):
    """List blobs matching a pattern (case-insensitive)."""
    matching_blobs = []
...
def download_and_upload_hcris_to_blob(year):
    """Download HCRIS ZIP from CMS, extract, and upload CSVs to blob storage."""
    ...
    try:
        ...
        print('Extracting and uploading to blob storage...')
        
        # Extract ZIP in memory and upload each CSV to blob
        ...
        print('HCRIS data uploaded to blob storage.')
        return True
    except Exception as e:
...
def run_cms_pipeline():
	...
	if blob_exists(MASTER_PATH):
		print(f"{MASTER_PATH} found in blob storage; skipping master assembly.")
		cms_df = read_csv_from_blob(MASTER_PATH, dtype={'CCN': 'string'})
		...
		download_and_upload_hcris_to_blob(end_year)
		...
		for i in range(start_year, end_year + 1):
			...
			# Find matching blob (case-insensitive)
			matching_blobs = list_blobs_with_pattern(pattern)
			if not matching_blobs:
				raise FileNotFoundError(f"No blob found matching {pattern} (case-insensitive)")
			
			blob_name = matching_blobs[0]
			alpha = read_csv_from_blob(blob_name, header=None, dtype={'CCN': 'string'}, names=[
		...
		for i in range(start_year, end_year + 1):
			...
			blob_name = matching_blobs[0]
			numeric = read_csv_from_blob(blob_name, header=None, dtype={'CCN': 'string'}, names=[
				'Report Record Number', 'Worksheet Code', 'Line Number', 'Column Number', 'Value'
			])
			numeric['Form ID'] = (
		...
		# Write baseline cms CSV to blob storage
		write_csv_to_blob(cms_df, 'cms_master.csv', index=False)
		print("Created master CMS file in blob storage.")
	...
	# Process most recent file from blob storage
	alpha_pattern = f"hosp10_{latest_year}_alpha.csv"
	alpha_blobs = list_blobs_with_pattern(alpha_pattern)
	if not alpha_blobs:
		raise FileNotFoundError(f"No blob found matching {alpha_pattern}")
	alpha = read_csv_from_blob(alpha_blobs[0], header=None, dtype={'CCN': 'string'}, names=['Report Record Number', 'Worksheet Code', 'Line Number', 'Column Number', 'Value'])
	...
	numeric_blobs = list_blobs_with_pattern(numeric_pattern)
	if not numeric_blobs:
		raise FileNotFoundError(f"No blob found matching {numeric_pattern}")
	numeric = read_csv_from_blob(numeric_blobs[0], header=None, dtype={'CCN': 'string'}, names=['Report Record Number', 'Worksheet Code', 'Line Number', 'Column Number', 'Value'])
	...
	if blob_exists(MASTER_PATH):
		print(f'{MASTER_PATH} found in blob storage; loading existing master.')
		cms_df = read_csv_from_blob(MASTER_PATH, dtype={'CCN': 'string'})
	else:
		pass
	...
	write_csv_to_blob(cms_df, 'cms_master.csv', index=False) # Write copy of all updated data to blob
	timestamp = datetime.now().strftime("%m_%Y") # Create timestamp for snapshot version of cms master
	write_csv_to_blob(cms_df, f'cms_master_{timestamp}.csv', index=False) # Write timestamped copy of master snapshot to blob
	cms_dedupe_df = cms_dedupe_df.drop_duplicates(subset='CCN') # Dedupe on the CCN records
	write_csv_to_blob(cms_dedupe_df, 'cms_dedupe.csv', index=False) # Write copy of deduped, recent two years of data to blob
	print('Master CMS data updated in blob storage; dedupe file created.')

	# STEP 4 - GET GEOCODE DATA FOR DEDUPE SET AND CHECK CAH STATUS
	print('Getting geocode data.')
	# Load geocode data from blob storage (create empty if doesn't exist)
	if blob_exists('geocode_data.csv'):
		geocodes_df = read_csv_from_blob('geocode_data.csv', dtype={'CCN': 'string'})
	else:
		geocodes_df = pd.DataFrame(columns=['CCN', 'Latitude', 'Longitude'])
		write_csv_to_blob(geocodes_df, 'geocode_data.csv', index=False)
	...
	write_csv_to_blob(need_geocodes_df, 'need_geocodes.csv', index=False) # Write list of hospitals that need geocodes to blob
	...
	if need_geocodes_df.empty: # Check if we need to get geocodes
		...
		# Initialize raw_geocode_calls.csv in blob storage with headers
		raw_calls_df = pd.DataFrame(columns=['CCN', 'Latitude', 'Longitude'])
		write_csv_to_blob(raw_calls_df, GEOCODE_CALLS, index=False)
		...
		# Write all geocode results to blob storage
		raw_calls_df = pd.DataFrame(results)
		write_csv_to_blob(raw_calls_df, GEOCODE_CALLS, index=False)
	
		# Read files from blob storage
		raw_calls_df = read_csv_from_blob('raw_geocode_calls.csv', dtype={'CCN': 'string'})
		need_geocodes_df = read_csv_from_blob('need_geocodes.csv', dtype={'CCN': 'string'})
		geocodes_df = read_csv_from_blob('geocode_data.csv', dtype={'CCN': 'string'})
		...
		write_csv_to_blob(geocodes_df, 'geocode_data.csv', index=False) # Overwrite old geocode data in blob storage
		processed_ccns = set(geocodes_df['CCN'])
		need_geocodes_df = need_geocodes_df[~need_geocodes_df['CCN'].isin(processed_ccns)]
		write_csv_to_blob(need_geocodes_df, 'need_geocodes.csv', index=False)
		raw_calls_df = raw_calls_df[~raw_calls_df['CCN'].isin(processed_ccns)]
		write_csv_to_blob(raw_calls_df, 'raw_geocode_calls.csv', index=False)
	...
	# Load existing distance calculations if they exist in blob storage
	if blob_exists('inter_hospital_distances.csv'):
		dist_df = read_csv_from_blob(
	...
	if len(missing_pairs) > 0:
		...
		write_csv_to_blob(dist_df, 'inter_hospital_distances.csv', index=False)
		print(f'Appended {len(new_rows)} new distance records to blob storage.')
	else:
		print('Distances computed.')

	# Compute minimum neighbor distances
	dist = read_csv_from_blob('inter_hospital_distances.csv', dtype={'CCN1':'string','CCN2':'string'})
	...
	write_csv_to_blob(cms_dedupe_df, 'cms_dedupe.csv', index=False)
	...
	cms_master_df = read_csv_from_blob('cms_master.csv')
	cms_dedupe_df = read_csv_from_blob('cms_dedupe.csv')
	...
	write_csv_to_blob(cms_dedupe_df, 'cms_visuals.csv', index=False) # Write output to blob storage
	print('CMS production model run complete. All outputs saved to blob storage.')

if __name__ == "__main__":
	print("=" * 60)
	print("CMS HCRIS Data Pipeline - Container Version")
	print("=" * 60)
	try:
		run_cms_pipeline()
		print("=" * 60)
